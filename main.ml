...
let rec ajoutIteratifsTab (tas_tab:tas_min_tab) (el_list:cle list) =
  match el_list with
  | [] -> ()
  | tete::reste -> ajoutMinTab tas_tab tete ;
                  ajoutIteratifsTab tas_tab reste

  
  
let make_tas_tab (el_list:cle list) (taille_allouer:int) =
  let tab = Array.make taille_allouer None in
  let taille = List.length el_list in
  let le_return:tas_min_tab = (ref tab, ref 0) in
  ajoutIteratifsTab le_return el_list ;
  le_return 
...
(* let test_ajout =  *)
(*   (\* Création d'un tas *\) *)
(*   let tas_initial : tas_min_tab =  make_tas_tab [(conv (0,0,0,2)) ; (conv (0,0,0,6)) ; (conv (0,0,0,5)) ; (conv (0,0,0,10)) ; (conv (0,0,0,13)) ; (conv (0,0,0,7)) ; (conv (0,0,0,8)) ; (conv (0,0,0,12)) ; (conv (0,0,0,15)) ; (conv (0,0,0,14))] 0 in *)
